...
if (!class_exists('LP_Gateway_IDPay')) {
  ...
  class LP_Gateway_IDPay extends LP_Gateway_Abstract
  {
    ...
    public function process_payment($order)
    {
      ...
      $idpay = $this->get_checkout_payment_url();
      $gateway_url = $this->link;

      $json = array(
        'result' => $idpay ? 'success' : 'fail',
        'redirect' => $idpay ? $gateway_url : ''
      );
    ...
    public function get_checkout_payment_url()
    {
      if ($this->get_form_data()) {
        ...
        update_post_meta($order->id, __('idpay_transaction_id','learnpress-idpay'), $result->id);
